...
from shaper.models.dgcnn_modules.dgcnn_utils import get_knn_inds, construct_edge_feature_index
from shaper.nn.functional import pdist as pairwise_distance
...
class DGCNNCorCls(nn.Module):
    ...
    def __init__(self,
                 ...
                 dropout_prob=0.5,
                 with_transform=True):
        super(DGCNNCorCls, self).__init__()
    ...
    def forward(self, data_batch):
        ...
        xyz = x
        with torch.no_grad():
            pdist = pairwise_distance(xyz)
            knn_inds = get_knn_inds(pdist, self.k)
        # EdgeConvMLP
        features = []
        for edge_conv in self.mlp_edge_conv:
            x = construct_edge_feature_index(x, knn_inds)
            x = edge_conv(x)
            x, _ = torch.max(x, 3)
            features.append(x)
...
if __name__ == "__main__":
    ...
    dgcnn = DGCNNCorCls(in_channels, num_classes, with_transform=False).cuda()
