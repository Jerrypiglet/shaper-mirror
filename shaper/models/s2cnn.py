...
from s2cnn_modules import S2Convolution, SO3Convolution
from s2cnn_modules import s2_equatorial_grid, so3_equatorial_grid, so3_integrate
from s2cnn_modules.utils.pc2sph import PointCloudProjector


class S2CNN_Cls(nn.Module):
    def __init__(self, in_channels, out_channels,
                 band_width_in=30,
                 feature_channels=(100, 100),
                 band_width_list=(16, 10)):
        super(S2CNN_Cls, self).__init__()
        ...
        if self.use_normal:
            self.out_layer = nn.Linear(nfeature_out * 2, out_channels)
        else:
            self.out_layer = nn.Linear(nfeature_out, out_channels)
    ...
    def forward(self, x):
        ...
        cls_logits = self.out_layer(inter_feature)
        preds = {
...
class S2CNN_ClsLoss(nn.Module):
    def __init__(self):
        super(S2CNN_ClsLoss, self).__init__()
