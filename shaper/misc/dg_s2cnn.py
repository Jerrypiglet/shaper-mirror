...
class DGS2CNNCls(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        if self.transform_xyz:
            self.transform_input = TNet(3, 3, k=k)
    ...
    def forward(self, data_batch):
        ...
        if self.transform_xyz:
            trans_matrix_xyz = self.transform_input(new_xyz_flipped)
            new_xyz_flipped = torch.bmm(trans_matrix_xyz, new_xyz_flipped)
            end_points["trans_matrix_xyz"] = trans_matrix_xyz

        local_s2cnn_features_list.append(new_xyz_flipped)
