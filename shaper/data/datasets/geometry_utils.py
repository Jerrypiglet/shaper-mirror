...
def render_pts_with_label(out, pts, label, delete_img=False, base=0, point_size=8):
    ...
    cmd = 'RenderShape %s -l %s %s 1200 1200 -p %d > /dev/null' % (tmp_pts, tmp_label, out, point_size)
    call(cmd, shell=True)
...
def render_pts(out, pts, delete_img=False, point_size=8, point_color='FF0000FF'):
    ...
    cmd = 'RenderShape %s %s 1200 1200 -p %d -c %s > /dev/null' % (tmp_pts, out, point_size, point_color)
    call(cmd, shell=True)
...
def render_pts_with_feature(out, pts, features, delete_img=False, point_size=8 , fancy_kp=False, fancy_kp_num=20, fancy_kp_radius=0.02, normalize=False):
    ...
    if fancy_kp:
        kp_list = np.where(features>0.1)[0]
        num_kp = len(kp_list)
        more_pts = np.zeros((num_kp*fancy_kp_num, 3), dtype=np.float32)
        more_features = np.zeros((num_kp*fancy_kp_num), dtype=np.float32)
        ...
        for i, idx in enumerate(kp_list):
            for j in range(fancy_kp_num):
                ...
                more_features[i*fancy_kp_num+j] = features[idx]

        pts = np.concatenate((pts, more_pts), axis=0)
        features = np.concatenate((features, more_features), axis=0)

    color = np.zeros((features.shape[0],4))
    color[:,3]=features
    np.savetxt(tmp_label,color)

    export_pts(tmp_pts, pts)

    cmd = 'RenderShape %s %s 1200 1200 -p %d -f %s > /dev/null' % (tmp_pts, out, point_size, tmp_label)
    call(cmd, shell=True)
