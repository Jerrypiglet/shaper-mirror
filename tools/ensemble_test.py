...
from shaper_few_shot.data.build import build_dataset
...
def main():
    ...
    for cross_index in range(args.num_cross):
        ...
        # cls_logit_ensemble = np.mean(cls_logit_collection, axis=0)
        # pred_prob_ensemble = np_softmax(cls_logit_ensemble)
        # KL_divergence_collection = []
        # for replica_index in range(args.num_replicas):

        pred_labels = ensemble_test(cls_logit_collection, args.score_heur)
        overall_acc, acc_per_class, true_positive_per_class = evaluate_classification(dataset, pred_labels)
        eval_results = {
            "overall_acc": overall_acc,
            "acc_per_class": acc_per_class,
            "true_positive_per_class": true_positive_per_class,
        }
