...
class ShapeNet55Compare(Dataset):
    ...
    dataset_map = {
        ...
        "support": "support_files_smp{}_cross{}.txt",
    ...
    def __init__(self, root_dir, dataset_names, class_num_per_batch=10,
        ...
        if self.dataset_name in ["train", "valid"]:
            ...
            raise ValueError("Undefined dataset_name: {}.".format(self.dataset_name))
    ...
    def _load_dataset(self, dataset_name):
        if dataset_name == "support":
            file_name = self.dataset_map[dataset_name].format(self.num_per_class, self.cross_num)
        ...
        for fname in support_fname_list:
            ...
            with h5py.File(data_path, 'r') as fid:
                ...
                support_data_labels.append(fid['label'][:])
        ...
        for i in range(support_data_labels.shape[0]):
            support_data_points_per_class[support_data_labels[i][0]].append(support_data_points[i, ...])
            support_data_normals_per_class[support_data_labels[i][0]].append(support_data_normals[i, ...])

        if target_split_fname == support_split_fname:
            target_data_points_per_class = support_data_points_per_class.copy()
            target_data_normals_per_class = support_data_normals_per_class.copy()
        else:
            ...
            for fname in target_fname_list:
                ...
                with h5py.File(data_path, 'r') as fid:
                    ...
                    target_data_labels.append(fid['label'][:])
            ...
            for i in range(target_data_labels.shape[0]):
                target_data_points_per_class[target_data_labels[i][0]].append(target_data_points[i, ...])
                target_data_normals_per_class[target_data_labels[i][0]].append(target_data_normals[i, ...])
...
if __name__ == "__main__":
    root_dir = "/home/rayc/Projects/shaper/data/shapenet55_h5/shapenet55_fewshot"
    shapenet55 = ShapeNet55Compare(root_dir, ['target'], cross_num=2, batch_target_num=10)
    print('ShapeNet55 total data num: ', shapenet55.__len__())
    for k, v in shapenet55[0].items():
        print("{}: {}".format(k, v))
